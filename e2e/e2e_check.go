...
type switchGroup struct {
	Status struct {
		...
		Healthy  bool                      `json:"healthy"`
		Nodes    []switchGroupNodeStatus   `json:"nodes"`
		Switches []switchGroupSwitchStatus `json:"switches"`
	} `json:"status"`
}

type switchGroupNodeStatus struct {
	FabricNodeRef namedResource `json:"fabricNodeRef"`
}

type switchGroupSwitchStatus struct {
	SwitchRef namedResource `json:"switchRef"`
}
...
func parseFlags() config {
	...
	flag.IntVar(&cfg.expectedSwitchGroups, "expected-switchgroups", 3, "expected SwitchGroup resource count")
	flag.IntVar(&cfg.groupHashLength, "group-hash-length", 7, "expected hash length for switchgroup-derived node labels")
...
func runChecks(cfg config, deadline time.Time, rows *[]row) error {
	...
	*rows = append(*rows, row{"fabricnode interfaces", "match topology yaml", nicDetail, "PASS"})
	...
	stageLog("wait_switchgroups", colorize("start", colorBold))
	actualSwitchGroups, err := waitForExpectedCount(
		"wait_switchgroups",
		"switchgroups.unifabric.io",
		cfg.expectedSwitchGroups,
		deadline,
		time.Duration(cfg.sleepSeconds)*time.Second,
	)
	if err != nil {
	...
	*rows = append(*rows, row{"switchgroups count", strconv.Itoa(cfg.expectedSwitchGroups), strconv.Itoa(actualSwitchGroups), "PASS"})

	stageLog("check_switchgroup_membership", colorize("start", colorBold))
	groupDetail, err := waitForCheckPass(
		"check_switchgroup_membership",
		validateSwitchGroupMembership,
		deadline,
		time.Duration(cfg.sleepSeconds)*time.Second,
	)
	if err != nil {
	...
	*rows = append(*rows, row{
		...
		"leaf groups and spine group match expected topology",
		groupDetail,
		"PASS",
	})
	...
	*rows = append(*rows, row{"node topology labels", "leaf/spine labels match expected switchgroups", labelDetail, "PASS"})
...
func validateSwitchGroupMembership() (bool, string) {
	expected := map[string]struct{}{
		switchGroupKey(1, []string{"node-gpu-1", "node-gpu-2"}, []string{"switch-gpu-leaf1", "switch-gpu-leaf2"}): {},
		switchGroupKey(1, []string{"node-gpu-3", "node-gpu-4"}, []string{"switch-gpu-leaf3", "switch-gpu-leaf4"}): {},
		switchGroupKey(2, nil, []string{"switch-gpu-spine1"}):                                                     {},
	}

	var groups switchGroupList
	if err := fetchResourceJSON("switchgroups.unifabric.io", &groups); err != nil {
		return false, fmt.Sprintf("failed to read switchgroups: %v", err)
	}
	...
	for _, group := range groups.Items {
		if !group.Status.Healthy {
			errs = append(errs, fmt.Sprintf("tier=%d switches={%s}: group not healthy", group.Status.Tier, strings.Join(sortedUnique(switchRefNames(group.Status.Switches)), ",")))
		}
		actual[switchGroupKey(int(group.Status.Tier), fabricNodeRefNames(group.Status.Nodes), switchRefNames(group.Status.Switches))] = struct{}{}
	}
	if len(errs) > 0 {
		sort.Strings(errs)
		return false, strings.Join(errs, "; ")
	...
	if !stringSetEqual(actual, expected) {
		...
		for key := range actual {
			actualGroups = append(actualGroups, formatSwitchGroupKey(key))
		}
		sort.Strings(actualGroups)
		return false, fmt.Sprintf("switchgroup membership mismatch: actual=%v", actualGroups)
	}
	return true, "switchgroup membership matches expected topology"
}
...
func validateNodeTopologyLabels(groupHashLength int) (bool, string) {
	...
	leafGroupA := switchGroupLabelValue(1, []string{"switch-gpu-leaf1", "switch-gpu-leaf2"}, groupHashLength)
	leafGroupB := switchGroupLabelValue(1, []string{"switch-gpu-leaf3", "switch-gpu-leaf4"}, groupHashLength)
	spineGroup := switchGroupLabelValue(2, []string{"switch-gpu-spine1"}, groupHashLength)
...
func fabricNodeRefNames(resources []switchGroupNodeStatus) []string {
	names := make([]string, 0, len(resources))
	for _, resource := range resources {
		if resource.FabricNodeRef.Name != "" {
			names = append(names, resource.FabricNodeRef.Name)
		}
	}
	return names
}

func switchRefNames(resources []switchGroupSwitchStatus) []string {
	names := make([]string, 0, len(resources))
	for _, resource := range resources {
		if resource.SwitchRef.Name != "" {
			names = append(names, resource.SwitchRef.Name)
		}
	}
	return names
}

func switchGroupKey(tier int, nodes, switches []string) string {
	return fmt.Sprintf("tier=%d|nodes=%s|switches=%s", tier, strings.Join(sortedUnique(nodes), ","), strings.Join(sortedUnique(switches), ","))
}

func formatSwitchGroupKey(key string) string {
...
func switchGroupLabelValue(tier int, switchNames []string, hashLength int) string {
	names := sortedUnique(switchNames)
	return switchGroupNamePrefix(tier) + shortHash(strings.Join(names, ","), hashLength)
}
