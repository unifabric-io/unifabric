...
Optional environment variables:
  ...
  SWITCH_TOPOLOGY_ARTIFACT_DIR  Directory containing switch-resources.yaml. Default: ./.tmp
...
if [[ -z "${SWITCH_TOPOLOGY_ARTIFACT_DIR}" ]]; then
  SWITCH_TOPOLOGY_ARTIFACT_DIR="${REPO_ROOT}/.tmp"
fi
...
if [[ "\${container_status}" != "0" ]]; then
  ...
  switch_sudo docker ps -a --filter name='$(printf '%q' "${SWITCH_AGENT_CONTAINER_NAME}")' || true
...
container_status_line=\$(switch_sudo docker inspect \
  ...
  '$(printf '%q' "${SWITCH_AGENT_CONTAINER_NAME}")')
...
log "switch-agent deployed successfully for ${#SCALE_OUT_SWITCHES[@]} switches."
