...
usage() {
  cat <<'EOF'
Usage:
  bash e2e/topology/setup.sh [all|step1-install-topology|step2-setup-rdma-rxe|step3-install-monitoring-operators|step4-install-unifabric|step5-deploy-switch-agent|step6-uninstall-unifabric|step7-deploy-hsflowd|step8-install-spiderpool-macvlan] [args...]
...
Examples:
  ...
  bash e2e/topology/setup.sh step8-install-spiderpool-macvlan
EOF
}
...
case "${stage}" in
  ...
  step6-uninstall-unifabric)
    ...
    ;;
  step7-deploy-hsflowd)
    run_step deploy-hsflowd.sh "$@"
    ;;
  step8-install-spiderpool-macvlan)
