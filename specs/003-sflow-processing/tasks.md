...
- [X] T002 [P] Move the `flows_raw` initialization schema from `1.sql` into the Go-based ClickHouse migration.
...
- [X] T031 [US1] Verify the switch and workload dashboards query only fields provided by the Go-based ClickHouse migration in `chart/files/switch-sflow.json` and `chart/files/workload-sflow.json`.
