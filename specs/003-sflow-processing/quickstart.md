...
Create the `flows_raw` table using the committed schema:

```bash
clickhouse-client --multiquery < docs/sql/sflow-flows-raw.sql
```

The schema must include source and destination Kubernetes Pod, node, and top-level owner fields because the dashboards query those columns directly.
