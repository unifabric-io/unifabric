# Tasks: SwitchGroup Scale-Out Topology Discovery
...
**Organization**: Tasks are grouped by phase and mapped to the single P1 user story so the SwitchGroup topology path can be implemented and validated as one independently testable increment.
...
- [x] T005 [P] Define the `SwitchGroup` API resource in `pkg/api/v1beta1/switchgroup_types.go`
- [x] T006 Update API registration and generated CRD outputs in `pkg/api/v1beta1/crd_types.go`, `pkg/api/v1beta1/zz_generated.deepcopy.go`, `chart/crds/unifabric.io_switches.yaml`, and `chart/crds/unifabric.io_switchgroups.yaml`
...
- [x] T010 Wire the controller startup path so SwitchGroup discovery becomes the active scale-out topology path in `pkg/controller/controller.go` and `pkg/controller/scaleoutgroup/scaleoutgroup.go`

**Checkpoint**: Foundation ready. `Switch`, `SwitchGroup`, config, chart values, and controller wiring are in place so story implementation can start.

---

## Phase 3: User Story 1 - Use Topology-Aware Scheduling in a GPU Cluster (Priority: P1) 🎯 MVP

**Goal**: Replace leaf-only scale-out discovery with a SwitchGroup-based path that ingests switch connectivity, computes leaf/spine/core topology, and writes stable scheduling labels.

**Independent Test**: In a cluster with known node-to-leaf and switch-to-switch connectivity, the controller can ingest switch snapshots, maintain `Switch` and `SwitchGroup` resources, and write stable leaf/spine/core labels back to participating GPU nodes.
...
- [x] T012 [P] [US1] Add controller unit tests for switch status ingestion, `SwitchGroup` reconciliation, and node label cleanup in `pkg/controller/switchtopology/switchtopology_test.go`
- [x] T013 [US1] Add an e2e scenario for switch subscriptions, `SwitchGroup` creation, and node label projection in `e2e/e2e_check.go` and `e2e/topology/`
...
- [x] T017 [P] [US1] Refine `pkg/topology/topology.go` so it consumes `FabricNode` and `Switch` inputs and emits deterministic SwitchGroup-ready topology outputs in `pkg/topology/topology.go`
...
- [x] T020 [US1] Implement `SwitchGroup` status projection for tier, switch refs, optional node refs, and health in `pkg/controller/switchtopology/projection.go` (depends on T019)
...
- [x] T025 [US1] Stop overlapping `ScaleOutLeafGroup` leaf output ownership when the SwitchGroup path is active in `pkg/controller/scaleoutgroup/scaleoutgroup.go` and `pkg/controller/controller.go` (depends on T021)

**Checkpoint**: User Story 1 is functional. The cluster can ingest switch snapshots, compute `SwitchGroup` topology, and write stable scale-out labels without conflicting leaf outputs.
...
- [x] T026 Update rollout and migration documentation in `docs/design/scaleout-topology.md`, `docs/design/scaleoutleafgroup.md`, and `specs/002-switchgroup-topology/quickstart.md`
- [x] T027 Run `make crd` to refresh `pkg/api/v1beta1/zz_generated.deepcopy.go`, `chart/crds/unifabric.io_switches.yaml`, and `chart/crds/unifabric.io_switchgroups.yaml`
...
- Status projection, label projection, observability, and deprecation handling (`T020`-`T025`) follow the main reconcile loop.
...
Task: "T017 Refine pkg/topology/topology.go for SwitchGroup-ready outputs"
```
...
5. Demo the SwitchGroup path before moving to polish tasks.
...
3. Add `SwitchGroup` status projection and Node label cleanup.
...
2. After Phase 2 completes:
   ...
   - Developer C: observability, deprecation handling, and validation (`T022`-`T031`)
...
- `ScaleOutLeafGroup` is treated as a deprecated path in this feature; tasks keep its shutdown and migration handling explicit.