...
本文讨论 Unifabric 中的 sFlow 信息处理方案。交换机侧将 sFlow v5 datagram 推送到独立的 `sflow` collector。collector 解码采样流记录，用源端和目的端 IP 匹配当前 Kubernetes Pod 信息，补充 Pod、Node 和顶层 workload owner 字段，然后写入 ClickHouse `flows_raw` 表。`chart/files/switch-sflow.json` 和 `chart/files/workload-sflow.json` dashboard 直接查询这张表。
...
- `flows_raw` 的初始化 SQL 位于 `docs/sql/sflow-flows-raw.sql`。
...
- 风险是 dashboard 与 ClickHouse schema 不一致。缓解方式是在实现阶段核对 `switch-sflow.json`、`workload-sflow.json` 的 raw SQL 只使用 `docs/sql/sflow-flows-raw.sql` 中存在的字段。
...
提交的初始化 schema 位于 `docs/sql/sflow-flows-raw.sql`。它创建 `default.flows_raw`，包含基础流字段，以及源端和目的端的 Kubernetes Pod、Node、顶层 owner 字段。
...
- 核对 `switch-sflow.json` 和 `workload-sflow.json` 的 raw SQL 只使用 `docs/sql/sflow-flows-raw.sql` 中存在的字段。
...
- `flows_raw` 表字段可以支撑 switch 和 workload sFlow dashboard 查询。
