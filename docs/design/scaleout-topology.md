...
- One risk is that auto-generated certificates rotate unexpectedly during `helm upgrade`. The mitigation is for the chart to reuse existing Secrets through `lookup` when auto-generation is enabled, instead of re-issuing them on every render.
...
7. Support ignore patterns for ports such as `mgmt*`, `Management*`, and `oob*`.
...
- When auto-generation is enabled, the chart should use Helm `genSelfSignedCert` during installation to generate the two self-signed certificates and use `lookup` to reuse any existing Secret, so that `helm upgrade` does not reissue certificates on every render.
...
- The Controller-side Secret is mounted directly by the chart. The switch-agent certificate bundle is generated by the chart and then exported through `NOTES.txt` or documentation so it can be copied to each switch.
...
- Unit tests for the switch-side LLDP JSON0 parser, covering ignored ports, missing fields, and case normalization.
