...
4. The Controller writes the final topology results back to Kubernetes Node labels according to `topoDiscovery.scaleOut.nodeLabel.keyTemplate` in the chart.
...
| `metadata.annotations[unifabric.io/switch-neighbors]` | JSON string array containing only directly connected switches; Kubernetes Node names are forbidden. When the key exists on every Switch of a role, it selects semi-automatic topology; key presence counts even when the value is empty or `[]`. |
...
- Verify that GPU Node topology labels are rendered from `topoDiscovery.scaleOut.nodeLabel.keyTemplate` and use stable `tier${N}-group${M}` values.
