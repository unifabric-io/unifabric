...
The current `ScaleOutLeafGroup` model can only express the direct leaf layer for a node. What this design adds is a unified control-plane path for leaf, spine, and core topology. The Controller continuously consumes switch neighbor data, maintains topology groups internally, and writes the results back to Node labels.
...
- `ScaleOutLeafGroup` enters a deprecated state. New scale-out topology functionality is designed around `Switch` inputs and Node label outputs, and `ScaleOutLeafGroup` will not be extended further.
- The `Switch` CRD and the switch topology discovery controller path form the new implementation. If a migration switch is kept during implementation, it is only for gradual rollout and rollback and should not become a long-term dual-write mode.
...
The switch-driven path is intended to replace `ScaleOutLeafGroup` for scale-out topology. The rollout sequence should keep that ownership boundary explicit.
...
During migration, `ScaleOutLeafGroup` should be treated as a legacy compatibility path. It may remain installed in the API, but once the switch-driven path is enabled it should no longer own scale-out labels or group membership for the same nodes.
...
After migration, this controller becomes the only writer for scale-out topology labels and is responsible for writing leaf, spine, and core labels. `ScaleOutLeafGroup` no longer owns scale-out topology discovery.
...
- Migration configuration tests to verify that when switch-driven discovery is enabled and `leafGroups` is disabled, the Controller processes the switch-driven path and no longer writes `ScaleOutLeafGroup`.
...
- Verify that when switch-driven discovery is enabled and `leafGroups` is disabled, leaf labels are generated by the switch-driven path and `ScaleOutLeafGroup` is no longer written.
...
- After migration completes, `ScaleOutLeafGroup` no longer owns scale-out topology representation. Leaf, spine, and core results are expressed through Node labels.
