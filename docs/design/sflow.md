...
`chart/files/switch-sflow.json`, `chart/files/workload-sflow.json`, and
`chart/files/workload-pair-sflow.json` query this table directly.
...
- Endpoints that do not match Pod IPs keep empty Kubernetes fields and do not
  block row writes.
- `flows_raw` is created and maintained by the ClickHouse schema migration that
  runs during sFlow collector startup.
...
- Risk: dashboards may drift from the ClickHouse schema. Mitigation: verify
  that `switch-sflow.json`, `workload-sflow.json`, and
  `workload-pair-sflow.json` raw SQL only use fields created by the Go
  migration.
...
The sFlow collector runs built-in ClickHouse schema migrations during startup.
The first migration uses date-based version `202606040001` and creates the
target database, `__unifabric_sflow_schema_migrations`, and `flows_raw`.
Multiple sFlow replicas serialize migration execution with a Kubernetes Lease,
so future schema changes are not applied concurrently. `flows_raw` contains
base flow fields plus source and destination Kubernetes Pod, Node, and top-level
owner fields.
...
- Check that `switch-sflow.json`, `workload-sflow.json`, and
  `workload-pair-sflow.json` raw SQL only use fields created by the Go
  migration.
...
- Base flow records can still be written to ClickHouse when Pod IP matching or
  owner lookup fails.
- `flows_raw` fields support the switch, workload, and workload pair sFlow
  dashboard queries.
...
- The original `1.sql` has moved into the Go-based ClickHouse migration.
