...
本文讨论一套面向 scale-out 网络的 leaf、spine、core 拓扑识别方案。交换机侧运行轻量级 switch agent 周期性采集 LLDP 邻居信息，Unifabric Controller 主动发起 gRPC 长连接订阅这些邻居信息，并将结果写入 `Switch` CRD 的 `status`。随后，Controller 再结合节点侧 `FabricNode` 记录的主机网卡信息，计算完整的 scale-out 网络拓扑，并将 GPU 节点所属的 leaf、spine、core 层级写回 Kubernetes Node label。这里使用 `Switch` 和 `SwitchGroup` 作为核心资源模型，用来替代现有 `ScaleOutLeafGroup` 对 scale-out 拓扑的表达。

现有 `ScaleOutLeafGroup` 只能表达节点直连 leaf 这一层结果。本文补上的，是一条面向 leaf、spine、core 的统一控制面链路。Controller 持续消费交换机邻居数据，维护拓扑分层，并把结果写回 `SwitchGroup` 和 Node label。
...
这也是 `ScaleOutLeafGroup` 的边界。它适合表达 leaf 分组，不适合承载 spine、core 结果，也不能直接表达交换机与节点之间的引用关系。只靠这一路输入，Kueue 等调度系统仍然拿不到一组稳定、可复用的 scale-out 拓扑结果。

因此，本提案把交换机侧的 LLDP 邻居信息接入 Controller，补齐 switch-to-switch 和 switch-to-node 关系，再与节点侧信息一起计算整张 scale-out 拓扑。顺着这组输入，Controller 可以统一生成 leaf、spine、core 对应的 `SwitchGroup`，回写 Node label，并把 `ScaleOutLeafGroup` 逐步迁移出这条链路。与此同时，方案仍然需要控制实现复杂度，避免让每台交换机都携带单独的 Kubernetes 凭据或复杂的 per-switch TLS 配置。
...
3. `SwitchTopologyDiscoveryController` 以 `FabricNode` 和 `Switch` 为输入，计算 GPU 节点对应的 leaf、spine、core 层级，并维护 `SwitchGroup`。
4. Controller 按 chart 中的 `topologyLabels` 配置，将最终拓扑结果写回 Kubernetes Node label。

迁移完成后，scale-out 拓扑发现和标签写回统一由 `SwitchTopologyDiscoveryController` 承担，`ScaleOutLeafGroup` 不再作为目标资源模型。Controller 通过 `Switch.spec` 获取拨号目标，通过全局 pinned mTLS 建立安全连接，并将返回快照中的 `switch_name` 持久化到 `Switch.status.hostname`，再把这个上报 hostname 作为交换机别名来源，用于关联交换机侧与节点侧的拓扑数据。
...
- `ScaleOutLeafGroup` 进入废弃状态。新的 scale-out 拓扑能力统一围绕 `Switch` 与 `SwitchGroup` 设计，不再继续扩展 `ScaleOutLeafGroup`。
- `SwitchGroup` CRD、`Switch` CRD 和交换机拓扑发现控制器链路构成新的实现。如果实现阶段保留迁移开关，它只服务于灰度迁移和回滚，不作为长期双写方案。
- 迁移完成后关闭 `scaleOutDiscovery.leafGroups.enabled`，停止对 `ScaleOutLeafGroup` 的写入。
- `SwitchGroup` 用于统一表达 leaf、spine、core 三类拓扑性能域，分层语义参考 Volcano 的网络拓扑层级设计。是否需要引入基于 CIDR 的分组能力，以及未来是否增加总览型 `Topology` CRD，仍属于后续演进议题。

### 风险与缓解

- 风险是长连接中断或交换机侧采集异常后，`Switch.status` 中保存的邻居数据变旧，进而影响 `SwitchGroup` 和 Node label 的更新时效。缓解方式是 Controller 侧维护指数退避重连、周期性 resync，并通过 `status.healthy` 和 conditions 明确暴露数据新鲜度与连接状态。
...
- 风险是自动生成证书在 `helm upgrade` 时被非预期轮换。缓解方式是 Chart 在自动生成模式下使用 `lookup` 复用已存在的 Secret，避免重复签发。
...
7. 支持端口忽略配置，例如 `mgmt*`、`Management*`、`oob*`。
...
- 自动生成时，chart 应在安装阶段使用 Helm 的 `genSelfSignedCert` 生成两张自签名证书，并通过 `lookup` 复用已存在的 Secret，避免每次 `helm upgrade` 都重签证书。
...
- Controller 侧 Secret 直接由 chart 挂载。switch agent 侧证书包由 chart 生成后通过 `NOTES.txt` 或文档给出导出命令，再拷贝到交换机本地使用。
...
| `status.lldpNeighbors` | 按远端系统聚合后的 LLDP 邻居列表 |
| `status.lldpNeighbors[*].remoteSystemType` | 远端系统解析后是 Kubernetes Node 还是另一台交换机 |
| `status.lldpNeighbors[*].remoteSystemName` | 该邻居条目对应的远端主机名或交换机名，`links` 中的所有链路都共享这个名字 |
| `status.lldpNeighbors[*].links` | 同一个远端系统对应的一条或多条本地到远端端口映射 |
| `status.lldpNeighbors[*].links[*].localPort` | 本地交换机端口 |
| `status.lldpNeighbors[*].links[*].remotePortID` | 该链路对应的远端端口标识 |

### 拓扑计算流程

新增 `SwitchTopologyDiscoveryController`，作为 scale-out 网络拓扑的目标控制器，负责计算 leaf、spine、core `SwitchGroup`，维护分层结果，并写回节点标签。

迁移完成后，由该控制器统一维护 `Switch` 和 `SwitchGroup`，并负责 leaf、spine、core 标签写回。`ScaleOutLeafGroup` 不再承担 scale-out 拓扑发现职责。
...
4. 针对每个 role 对应的拓扑图，创建、更新或删除 leaf、spine、core `SwitchGroup`。
...
`SwitchGroup` 是 cluster-scoped 资源，用于统一表达 scale-out 网络中的拓扑性能域。它参考 Volcano 的网络拓扑层级语义，但不直接保存组与组之间的成员引用。对于本提案，`status.role` 表示该组来自哪一类交换机拓扑，`status.tier: 1` 表示 leaf，`status.tier: 2` 表示 spine，`status.tier: 3` 表示 core。Controller 会先按 `Switch.spec.role` 对交换机分批，再分别计算拓扑，因此 `ScaleOut`、`ScaleUp`、`Storage` 不会混在同一张拓扑图里。`SwitchGroup` 由 Controller 自动创建和维护，不定义业务 `spec` 字段；所有由 Controller 计算得到的分层结果都写在 `status` 中。每个对象描述当前层级对应的 Switch CR 引用、可选的 FabricNode CR 引用以及健康状态。如果当前层级没有匹配到任何 FabricNode，则不写 `status.nodes` 字段。当前 Node label 写回链路仍然只消费 `ScaleOut` 这批结果。
...
| 字段 | 含义 |
| --- | --- |
| `status.role` | 该组所属的交换机角色：`ScaleOut`、`ScaleUp` 或 `Storage`。Controller 会按 role 分批计算拓扑。 |
| `status.tier` | 拓扑层级。当前约定 `1=leaf`、`2=spine`、`3=core` |
| `status.switches` | 支撑该 `SwitchGroup` 的 Switch CR 引用列表 |
| `status.switches[*].switchRef.name` | 被纳入该 `SwitchGroup` 的 Switch CR 名称。由于 `Switch` 是 cluster-scoped 资源，当前仅需 `name` 即可唯一定位 |
| `status.nodes` | 当前落在该 `SwitchGroup` 下的 FabricNode CR 引用列表。若当前层级没有匹配到任何 FabricNode，则该字段可以省略 |
| `status.nodes[*].fabricNodeRef.name` | 被纳入该 `SwitchGroup` 的 FabricNode CR 名称。由于 `FabricNode` 是 cluster-scoped 资源，当前仅需 `name` 即可唯一定位 |
| `status.healthy` | 当前成员关系和节点映射是否足以支撑标签写回与对账 |

在 leaf `SwitchGroup` 中，通常会出现 `status.nodes`。在 spine 或 core `SwitchGroup` 中，如果该层级没有直接对应的 FabricNode，或者该数组为空，则不在 `status` 中显示 `nodes` 字段。
...
对于 `SwitchGroup`，`metadata.name` 固定使用短 hash，例如 `7f3a9c2`。为避免不同层级或不同交换机集合发生命名冲突，生成该短 hash 时应把 `status.tier` 和规范化后的交换机集合一起作为输入。层级信息本身通过 `status.tier` 表达，而不是体现在 `metadata.name` 前缀中。
...
无论使用哪种格式，leaf、spine、core 的 label value 都应基于同样的交换机集合生成，并保持稳定可复现。Controller 直接按所选格式写回 Node label，不再将 label key 或 label value 再保存到 `SwitchGroup.status`。

同时在日志和指标中输出该 group 对应的交换机集合，便于从 label value 反查实际 switch 列表。`SwitchGroup.status.switches` 应暴露完整的 Switch CR 引用列表，必要时也可通过 `status.nodes` 对账该层级覆盖的 FabricNode。
...
- 拓扑计算摘要，包含 switch 数、node 数、link 数，以及 `SwitchGroup` 与节点 label 更新结果。

### 测试计划

#### 单元测试

- Switch-side LLDP JSON0 parser 单元测试，覆盖端口忽略、字段缺失和大小写标准化。
- proto 编解码测试和快照 generation 去重/乱序处理测试。
- 迁移配置测试，验证启用 `SwitchGroup` 链路并停用 `leafGroups` 后，只处理 `Switch` / `SwitchGroup` 逻辑，不再写入 `ScaleOutLeafGroup`。
- `Switch.spec.mgmtIP` / `spec.grpcPort` 变更触发重拨逻辑测试。
- 拓扑构建、`SwitchGroup` 生成与节点 label 写回逻辑测试。

#### e2e 测试

- 验证启用 `SwitchGroup` 链路并停用 `leafGroups` 后，leaf 标签由 `SwitchGroup` 链路生成，且不会继续写入 `ScaleOutLeafGroup`。
...
- 验证 leaf、spine、core `SwitchGroup` 与 GPU Node 的拓扑 label 是否正确，校验所使用的 label key 应来自 chart 的 `topologyLabels.scaleOutLeaf`、`topologyLabels.scaleOutSpine` 和 `topologyLabels.scaleOutCore` 配置，label value 则应符合所选格式要求，即名字格式或短 hash 格式。
...
- 迁移完成后，`ScaleOutLeafGroup` 不再承担 scale-out 拓扑表达职责，leaf、spine、core 结果都可以由 `SwitchGroup` 和 Node label 表达。
- Controller 能够稳定维护到交换机的 gRPC 订阅连接，并在断连后自动恢复。
- `Switch.status` 可以持续反映最新 LLDP 快照、连接状态和健康状态。
- `SwitchGroup` 可以持续反映 leaf、spine、core 的成员关系和健康状态。
