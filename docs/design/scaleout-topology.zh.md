...
4. Controller 按 chart 中的 `topoDiscovery.scaleOut.label.keyTemplate`，将最终拓扑结果写回 Kubernetes Node label。
...
| `metadata.annotations[unifabric.io/neighbors]` | 直连交换机组成的 JSON 字符串数组；同 role 的全部 Switch 都存在该 key 时选择半自动拓扑，值为空或 `[]` 也算存在 |
...
在内置 RoCE 发现模式下，主机 LLDP 提供 Node 到 leaf 的连接。半自动发现时，用户只创建更高层的 Switch 资源，并确保每一个资源都存在 `unifabric.io/neighbors` key。Node LLDP 构造虚拟 leaf，annotation 再把这些 leaf 连接到声明的上层交换机。只要同一有效 role 中有一个 Switch 缺少该 key，Controller 就改用全自动交换机发现并忽略所有 neighbors annotation。
...
annotation 必须写成带引号的 JSON 字符串数组，因为 Kubernetes annotation 的 value 只能是字符串。邻居名称既可以引用 FabricNode LLDP 发现出的虚拟 leaf，也可以引用相同有效 role 下另一个带 annotation 的 Switch。链路按无向边处理，只在一端填写即可；重复声明会自动去重。空名称、自引用、JSON 格式错误和无法解析的引用都会中止本轮协调并保留已有拓扑 labels。annotation key 本身决定是否进入半自动模式，因此空值和 `[]` 也算存在；该模式不要求也不使用 `spec.mgmtIP`、gRPC 字段或 `status.lldpNeighbors` 作为拓扑输入。
...
- 验证 GPU Node 拓扑 label 是否由 `topoDiscovery.scaleOut.label.keyTemplate` 渲染，并使用稳定的 `tier${N}-group${M}` value。
