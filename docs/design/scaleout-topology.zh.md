...
现有 `ScaleOutLeafGroup` 只能表达节点直连 leaf 这一层结果。本文补上的，是一条面向 leaf、spine、core 的统一控制面链路。Controller 持续消费交换机邻居数据，在内部维护拓扑分组，并把结果写回 Node label。
...
因此，本提案把交换机侧的 LLDP 邻居信息接入 Controller，补齐 switch-to-switch 和 switch-to-node 关系，再与节点侧信息一起计算整张 scale-out 拓扑。顺着这组输入，Controller 可以统一生成 leaf、spine、core 对应的 Node label，并把 `ScaleOutLeafGroup` 逐步迁移出这条链路。与此同时，方案仍然需要控制实现复杂度，避免让每台交换机都携带单独的 Kubernetes 凭据或复杂的 per-switch TLS 配置。
...
- `ScaleOutLeafGroup` 进入废弃状态。新的 scale-out 拓扑能力围绕 `Switch` 输入和 Node label 输出设计，不再继续扩展 `ScaleOutLeafGroup`。
- `Switch` CRD 和交换机拓扑发现控制器链路构成新的实现。如果实现阶段保留迁移开关，它只服务于灰度迁移和回滚，不作为长期双写方案。
...
迁移完成后，由该控制器统一负责 leaf、spine、core 标签写回。`ScaleOutLeafGroup` 不再承担 scale-out 拓扑发现职责。
...
- 迁移配置测试，验证启用交换机拓扑链路并停用 `leafGroups` 后，只处理交换机拓扑逻辑，不再写入 `ScaleOutLeafGroup`。
- `Switch.spec.mgmtIP` / `spec.grpcPort` 变更触发重拨逻辑测试。
- 拓扑构建、拓扑分组投影与节点 label 写回逻辑测试。

#### e2e 测试

- 验证启用交换机拓扑链路并停用 `leafGroups` 后，leaf 标签由交换机拓扑链路生成，且不会继续写入 `ScaleOutLeafGroup`。
...
- 迁移完成后，`ScaleOutLeafGroup` 不再承担 scale-out 拓扑表达职责，leaf、spine、core 结果都通过 Node label 表达。
