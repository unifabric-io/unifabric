...
`TopologyStatusController` 根据 Topology 名称选择生成后的内部 label 模板。Scale-up 和 scale-out mode 分别决定 NVIDIA Topograph 是否把对应拓扑映射到 Unifabric 配置的 label key：只有 scale-up 选择 `nv-topograph` 时 accelerator 才使用 `topoDiscovery.scaleUp.nodeLabel.keyTemplate`，只有 scale-out 选择 `nv-topograph` 时 leaf、spine、core 才使用 `topoDiscovery.scaleOut.nodeLabel.keyTemplate`。未交给 NVIDIA 管理的拓扑继续使用 Topograph 原生默认 key，因此不会与该拓扑选定的 writer 争用 Unifabric label。Helm values 中的字符串不会自动进行第二次模板求值，因此 Chart helper 必须先校验受限模板，再通过 `tpl` 和只包含 `Tier` 的上下文生成具体 label key。Topology CRD 不维护另一套 NVIDIA label 映射。
...
在内置 RoCE 模式下，没有交换机侧 LLDP 的 Switch 还可以通过通用的 `unifabric.io/switch-neighbors` annotation 声明物理交换机邻居，例如 `unifabric.io/switch-neighbors: '["leaf1", "leaf2"]'`。fabric 类型由 `spec.role` 决定，所以不同 role 共用同一个 annotation key。主机 LLDP 提供 Node 到 leaf 的连接，该 annotation 只提供 Switch 到 Switch 的连接。**数组中只能填写交换机名称，绝不能填写 Kubernetes Node 名称；Node LLDP 发现出的虚拟 leaf 仍表示交换机，因此可以填写。** 自动发现随后写入 Node tier labels 和 Switch `domain` labels。邻居必须引用同 role 的 Switch 资源或节点发现出的虚拟 leaf 交换机，无向链路只需在一端声明。
...
7. 在 `e2e/topology` 的 NVAIR 环境运行三套相互隔离的流程。第一套启用 mTLS 和
   switch-agent LLDP `roce-lldp-full-auto` 发现，验证 FabricNode、Switch、Node/Switch labels 和
   `Topology/scaleout` 的完整链路，以及 Controller 重启和删除 Topology CR 后的
   finalizer 重置。完成后卸载 Chart，并清理 CR、CRD 和受管 labels。第二套重新安装 Chart，
   禁用交换机订阅，仅使用节点 LLDP 和 `unifabric.io/switch-neighbors` 交换机邻居
   annotation 完成 `roce-lldp-node-only` 发现，并验证相同的最终拓扑。完成后再次卸载和清理。
   第三套 `nv-topograph` 场景重新安装 Chart，保持 `scaleUp.mode=manual` 且不部署
   NVIDIA Topograph，通过手工 Node accelerator labels 模拟其 Scale-up 输出，验证 `Topology/scaleup`
   性能域和 Node 分组，以及清空 labels 后保留空 CR。该流程明确模拟 Topograph 的
   label 输出，不宣称验证了真实 InfiniBand 或 NVIDIA Topograph 数据源。
