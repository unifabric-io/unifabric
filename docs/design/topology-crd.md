...
In the built-in RoCE mode, a Switch without switch-side LLDP may additionally declare physical Switch neighbors with the generic `unifabric.io/switch-neighbors` annotation, for example `unifabric.io/switch-neighbors: '["leaf1", "leaf2"]'`. `spec.role` selects the fabric, so the annotation key is shared across roles. Host LLDP supplies Node-to-leaf links, while this annotation supplies Switch-to-Switch links. Its entries are switch names only—never Kubernetes Node names. A node-discovered synthetic leaf still represents a switch and is therefore a valid entry. Auto-discovery then writes the Node tier labels and Switch `domain` labels. Neighbor references must resolve to same-role Switch resources or node-discovered synthetic leaf switches, and one side of an undirected link is sufficient.
...
7. Run three isolated flows in the NVAIR environment under `e2e/topology`. The first enables
   mTLS and `roce-lldp-full-auto` switch-agent LLDP discovery and validates the complete FabricNode, Switch,
   Node/Switch-label, and `Topology/scaleout` chain, including Controller restart and the
   finalizer-driven reset after deleting the Topology CR. Uninstall the Chart and remove CRs,
   CRDs, and managed labels afterward. Reinstall the Chart for a second flow with switch subscription
   disabled, using only Node LLDP and switch-only `unifabric.io/switch-neighbors` annotations for
   `roce-lldp-node-only` discovery, and verify the same final topology. Uninstall and clean again.
   For the third `nv-topograph` flow, reinstall with `scaleUp.mode=manual` and without the NVIDIA
   Topograph components, then manually apply Node accelerator labels that simulate its Scale-up output.
   Verify the `Topology/scaleup` domains and Node groups and an empty retained CR after labels are
   removed. This flow explicitly simulates Topograph's label output and does not claim to validate
   a real InfiniBand or NVIDIA Topograph data source.
