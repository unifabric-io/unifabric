...
| Helm value | 用途 |
| --- | --- |
| `switchTopologyDiscovery.enabled` | 开启基于交换机邻居的拓扑发现。 |
| `switchTopologyDiscovery.ignoreSwitchPorts` | 可选：由 controller 在拓扑计算前忽略交换机本地端口，默认 `mgmt*`、`Management*`、`oob*`。 |
...
- 确认 `switchTopologyDiscovery.enabled=true`。
