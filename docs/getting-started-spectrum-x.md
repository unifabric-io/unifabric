...
- Nodes receive the `scale-out.unifabric.io/tier-N` topology labels used by schedulers.
...
## Configure Spectrum-X Topology Discovery

Complete the [basic installation](./getting-started.md). This scenario adds only the NetQ
credentials and the NVIDIA Topograph NetQ provider configuration.
...
All other installation, image, and observability settings remain those from the basic installation.
...
- Confirm that `fabricNode.scaleOutInterfaceSelector` matches the RDMA NIC name or CIDR.
