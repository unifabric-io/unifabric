...
- Node 会获得供调度系统使用的 `scale-out.unifabric.io/tier-N` 拓扑 label。
...
先按照[基础安装](./getting-started.zh.md)完成通用组件安装。本场景只补充 NetQ 凭据和
NVIDIA Topograph 的 NetQ provider 配置。
