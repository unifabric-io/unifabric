...
func buildTopologyInputsForRole(role v1beta1.SwitchRole, selectorCfg config.ControllerNodeTopologyConfig, fabricNodes []v1beta1.FabricNode, switches []v1beta1.Switch) topologyInputs {
	...
	hostNICs := buildHostNICLookup(fabricNodes)
	roleMatcher := newHostPeerRoleMatcher(selectorCfg)
	...
	for _, sw := range switches {
		...
		for _, neighbor := range sw.Status.LLDPNeighbors {
			...
			for _, link := range switchNeighborLinks(neighbor) {
				if isHostPeer {
					matchedRole := roleMatcher.roleForHostLink(link.RemotePortID, lookupHostNIC(hostNICs, remoteSystemName, link.RemotePortID))
					if matchedRole != role {
						continue
					}
					participatingHostSet[remoteSystemName] = true
				}
				builder.addNeighbor(topology.LLDPNeighbor{
					LocalDeviceName:  sw.Name,
					LocalPort:        link.LocalPort,
					RemoteSystemName: remoteSystemName,
					RemotePortID:     link.RemotePortID,
				})
			}
		}
	...
	for _, node := range fabricNodes {
		...
		for _, nic := range nics {
			...
			builderBySwitch[switchName].addNeighbor(topology.LLDPNeighbor{
				LocalDeviceName:  switchName,
				LocalPort:        nic.LLDPNeighbor.Port,
				RemoteSystemName: node.Name,
				RemotePortID:     nic.Name,
			})
			matchedHost = true
		}
	...
	for _, switchName := range switchNames {
		...
		sort.Slice(builder.data.Neighbors, func(i, j int) bool {
			if builder.data.Neighbors[i].LocalPort != builder.data.Neighbors[j].LocalPort {
				return builder.data.Neighbors[i].LocalPort < builder.data.Neighbors[j].LocalPort
			}
			if builder.data.Neighbors[i].RemoteSystemName != builder.data.Neighbors[j].RemoteSystemName {
				return builder.data.Neighbors[i].RemoteSystemName < builder.data.Neighbors[j].RemoteSystemName
			}
			return builder.data.Neighbors[i].RemotePortID < builder.data.Neighbors[j].RemotePortID
		})
...
func buildHostNICLookup(fabricNodes []v1beta1.FabricNode) map[string]map[string]v1beta1.NicInfo {
	lookup := map[string]map[string]v1beta1.NicInfo{}
	for _, node := range fabricNodes {
		byName := map[string]v1beta1.NicInfo{}
		for _, nic := range node.Status.ScaleOutNics {
			if nic.Name != "" {
				byName[nic.Name] = nic
			}
		}
		for _, nic := range node.Status.StorageNics {
			if nic.Name != "" {
				byName[nic.Name] = nic
			}
		}
		lookup[node.Name] = byName
	}
	return lookup
}

func lookupHostNIC(hostNICs map[string]map[string]v1beta1.NicInfo, hostName, ifName string) *v1beta1.NicInfo {
	if ifName == "" {
		return nil
	}
	nicsByName, ok := hostNICs[hostName]
	if !ok {
		return nil
	}
	nic, ok := nicsByName[ifName]
	if !ok {
		return nil
	}
	copy := nic
...
func (b *topologySwitchBuilder) addNeighbor(neighbor topology.LLDPNeighbor) {
	if neighbor.LocalPort == "" || neighbor.RemoteSystemName == "" {
		return
	}

	key := fmt.Sprintf("%s|%s|%s", neighbor.LocalPort, neighbor.RemoteSystemName, neighbor.RemotePortID)
	if b.seen[key] {
		return
	}
	b.seen[key] = true
...
func switchNeighborLinks(neighbor v1beta1.SwitchLLDPNeighbor) []v1beta1.SwitchLLDPLink {
