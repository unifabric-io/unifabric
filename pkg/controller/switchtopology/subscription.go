...
func (m *subscriptionManager) handleSnapshot(ctx context.Context, switchName string, snapshot *switchagent.LLDPNeighborSnapshot, lastGeneration *uint64) (bool, error) {
	...
	neighbors, err := aggregateSnapshotNeighbors(snapshot, switchName, ignoreSwitchPorts, nodeNames, normalizedNodeNames)
	if err != nil {
	...
	m.log.Debug(
		...
		"aggregatedNeighborCount", len(neighbors),
	)
...
type switchLLDPNeighborAggregate struct {
	neighbor  v1beta1.SwitchLLDPNeighbor
	seenLinks map[string]bool
}

func aggregateSnapshotNeighbors(
	snapshot *switchagent.LLDPNeighborSnapshot,
	switchName string,
	ignoreSwitchPorts []string,
	nodeNames map[string]bool,
	normalizedNodeNames map[string]bool,
) ([]v1beta1.SwitchLLDPNeighbor, error) {
	aggregates := map[string]*switchLLDPNeighborAggregate{}

	for _, neighbor := range snapshot.GetLldpNeighbors() {
		if neighbor.GetLocalPort() == "" || neighbor.GetRemoteSystemName() == "" || neighbor.GetRemotePortId() == "" {
		...
		key := fmt.Sprintf("%s|%s", remoteSystemType, neighbor.GetRemoteSystemName())
		aggregate, ok := aggregates[key]
		if !ok {
			aggregate = &switchLLDPNeighborAggregate{
				neighbor: v1beta1.SwitchLLDPNeighbor{
		...
		link := v1beta1.SwitchLLDPLink{
			LocalPort:    neighbor.GetLocalPort(),
			RemotePortID: neighbor.GetRemotePortId(),
		}
		linkKey := fmt.Sprintf("%s|%s|%s", link.LocalPort, aggregate.neighbor.RemoteSystemName, link.RemotePortID)
		if aggregate.seenLinks[linkKey] {
			continue
		}
		aggregate.seenLinks[linkKey] = true
		aggregate.neighbor.Links = append(aggregate.neighbor.Links, link)
	}

	neighbors := make([]v1beta1.SwitchLLDPNeighbor, 0, len(aggregates))
	for _, aggregate := range aggregates {
		sort.Slice(aggregate.neighbor.Links, func(i, j int) bool {
			if aggregate.neighbor.Links[i].LocalPort != aggregate.neighbor.Links[j].LocalPort {
				return aggregate.neighbor.Links[i].LocalPort < aggregate.neighbor.Links[j].LocalPort
			}
			return aggregate.neighbor.Links[i].RemotePortID < aggregate.neighbor.Links[j].RemotePortID
		})
		neighbors = append(neighbors, aggregate.neighbor)
	}
