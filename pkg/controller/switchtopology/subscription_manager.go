...
func (m *subscriptionManager) handleSnapshot(ctx context.Context, switchName string, snapshot *switchagent.LLDPNeighborSnapshot, lastGeneration *uint64) (bool, error) {
	...
	neighbors, err := normalizeSnapshotNeighbors(snapshot, ignoreSwitchPorts, nodeNames, normalizedNodeNames)
	if err != nil {
...
func classifySubscriptionError(err error) string {
	...
	if code == codes.InvalidArgument {
		return v1beta1.SwitchReasonSnapshotRejected
	}
	if code == codes.FailedPrecondition || code == codes.Unauthenticated || code == codes.PermissionDenied {
