...
func (r *Reconciler) Reconcile(ctx context.Context, req reconcile.Request) (reconcile.Result, error) {
	...
	if r.reconcileCount == 0 {
		...
		for _, fabricNode := range fabricNodeList.Items {
			...
			if fabricNode.Status.NodeRole == v1beta1.NodeRoleStorage {
...
func (r *Reconciler) updateNodeLabel(ctx context.Context, nodeName, groupName string) error {
	leafLabelKey := r.cfg.TopologyLabels.ScaleOutLeaf
	if leafLabelKey == "" {
		leafLabelKey = config.DefaultLabelScaleOutLeaf
	}
	...
	finalLabelValue := node.Labels[leafLabelKey]
	switch groupName {
	case "":
		r.log.Debug("removing group label from node", "node", nodeName, "label", leafLabelKey)
		delete(node.Labels, leafLabelKey)
	case finalLabelValue:
		r.log.Debug("node topology label has no change, skipping", "node", nodeName, "label", leafLabelKey, "value", groupName)
		return nil
	default:
		r.log.Debug("updating group label to node", "node", nodeName, "oldGroup", finalLabelValue, "newGroup", groupName)
		node.Labels[leafLabelKey] = groupName
	}
	...
	r.log.Info("updated node label", "node", nodeName, "label", leafLabelKey, "value", groupName)
	return nil
}
