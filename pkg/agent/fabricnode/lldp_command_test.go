...
func TestResolveLldpCliInvocationFallsBackToPathWithoutSocket(t *testing.T) {
	originalLookup := commandLookup
	originalPathCheck := pathCheck
	t.Cleanup(func() {
		commandLookup = originalLookup
	...
	pathCheck = func(path string) error {
		switch path {
		case "/usr/local/sbin/lldpcli-1.0.16":
			return os.ErrNotExist
		default:
			return fmt.Errorf("unexpected path check for %s", path)
		}
	...
	command, args, err := resolveLldpCliInvocation(LldpCliOptions{
		BinaryPath:     "/usr/local/sbin/lldpcli-1.0.16",
		SocketPath:     "/run/lldpd.socket",
		FallbackToPath: true,
	})
	if err != nil {
	...
	if command != "/usr/sbin/lldpcli" {
		t.Fatalf("expected PATH lldpcli, got %s", command)
	}
	expectedArgs := []string{"-f", "json0", "show", "neighbors"}
	if len(args) != len(expectedArgs) {
		t.Fatalf("expected %d args, got %d", len(expectedArgs), len(args))
...
func TestResolveLldpCliInvocationDoesNotFallBackToPathWhenConfiguredSocketIsMissing(t *testing.T) {
	originalLookup := commandLookup
	originalPathCheck := pathCheck
	t.Cleanup(func() {
		commandLookup = originalLookup
	...
	commandLookup = func(name string) (string, error) {
		t.Fatalf("commandLookup should not be called when configured binary exists and socket is missing, got %s", name)
		return "", os.ErrNotExist
	}
	pathCheck = func(path string) error {
		switch path {
	...
	_, _, err := resolveLldpCliInvocation(LldpCliOptions{
		BinaryPath:     "/usr/local/sbin/lldpcli-1.0.16",
		SocketPath:     "/run/lldpd.socket",
		FallbackToPath: true,
	})
	if err == nil {
		t.Fatal("expected missing configured socket to fail")
	}
	if !strings.Contains(err.Error(), "lldp socket /run/lldpd.socket is unavailable") {
