...
type SwitchLLDPLink struct {
	LocalPort    string `json:"localPort"`
	RemotePortID string `json:"remotePortID"`
}

type SwitchLLDPNeighbor struct {
	...
	// RemoteSystemName identifies the remote system shared by all aggregated links.
	RemoteSystemName string `json:"remoteSystemName"`
	// Links aggregates one or more local-to-remote port pairs for the same remote system.
	// +optional
	Links []SwitchLLDPLink `json:"links,omitempty"`
}
...
type SwitchStatus struct {
	...
	LLDPNeighbors []SwitchLLDPNeighbor `json:"lldpNeighbors,omitempty"`
}
