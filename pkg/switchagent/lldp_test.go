...
func TestLldpCollectionOptionsUsesConfiguredSocketVersion(t *testing.T) {
	...
	if !options.FallbackToPath || options.FallbackToHost || options.UseHostNamespace {
		t.Fatalf("expected socket options with PATH fallback, got %#v", options)
	}
}
...
func TestLldpCollectionOptionsDefaultsTo116Socket(t *testing.T) {
	...
	if !options.FallbackToPath {
		t.Fatalf("expected PATH fallback, got %#v", options)
	}
}
